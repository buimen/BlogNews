<?php
	...
	$categoryID = $_GET['id'];
	$list = $category->getCategoryById($categoryID);

	$categoryName = isset($_POST['name']) ? $_POST['name'] : null;
	$categoryParent = isset($_POST['parent']) ? $_POST['parent'] : null;
	$memberEditCategory = 1;
	$date = getdate();
	$datetimeEditCategory = $date['year']. "-" .$date['mon']. "-" .$date['mday']. " - " .$date['hours']. "-" .$date['minutes']. "-" .$date['seconds'];
	if (isset($_POST['save'])) {
		$listSave = $category->setCategoryEdit($categoryID, $categoryName, $categoryParent, $memberEditCategory, $datetimeEditCategory);
		if ($list) {
			echo "<script>
		...
		<div class="container">
			...
			<div class="col-sm-10">
				<form method="post" action="" enctype="multipart/form-data">
					<div class="category">
						<input class="col-sm-12" type="text" name="name" placeholder="Tên chuyên mục..." value="<?php echo $list['categoryName'] ?>">
						<br/><br/>
						<input type="number" name="parent" min="0" max="10" value="<?php echo $list['categoryParent'] ?>">
					</div>
					<div class="bottom">
